"""Trust engine -- source credibility, span verification, cross-validation, and confidence scoring."""
...
# We split domain trust into tiers so the heuristic stays readable.
# These aren't perfect, but they're a decent prior before LLM assessment kicks in.
...
def credibility_scorer_node(state: AgentState) -> Dict[str, Any]:
    """Score each source on domain trust, authority, and content quality."""
    ...
    sources_text = []
    for s in sources[:15]:  # keep it short to avoid token bloat
        sources_text.append(
...
def _calculate_domain_trust(url: str) -> float:
    """Quick heuristic: known domains get a preset score, everything else gets 0.5."""
    try:
        parsed = urlparse(url)
...
def span_verify_node(state: AgentState) -> Dict[str, Any]:
    """Check each claim against actual evidence text to catch hallucinations."""
    ...
    for i in range(0, len(claims), batch_size):
        ...
        # Gather only the evidence relevant to this batch
        batch_evidence = []
    ...
    # Catch any claims that slipped through (LLM didn't mention them at all)
...
def cross_validate_node(state: AgentState) -> Dict[str, Any]:
    """Flag claims backed by 2+ independent sources -- these are our strongest facts."""
    ...
    for vc in verified_citations:
        ...
        if cv_info.get("cross_validated", False):
            ...
            if not supporting_sids:
                # Derive SIDs from EIDs when the LLM only gave us EIDs
...
def claim_confidence_scorer_node(state: AgentState) -> Dict[str, Any]:
    """Combine verification status, cross-validation, and source quality into per-claim confidence."""
    verified_citations = state.get("verified_citations") or []
