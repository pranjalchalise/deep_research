"""Batched trust engine -- combines multiple LLM calls to cut API costs (~60% fewer calls)."""
...
def batched_credibility_claims_node(state: AgentState) -> Dict[str, Any]:
    """Score source credibility AND extract claims in one LLM call to save tokens."""
    cfg = ResearchConfig()
...
def batched_verification_node(state: AgentState) -> Dict[str, Any]:
    """Verify, cross-validate, and score all claims in one LLM call."""
    cfg = ResearchConfig()
...
Verify each claim, checking for span matches and cross-validation."""
    ...
    # Claims the LLM didn't mention at all count as unverified
...
def full_trust_engine_batched(state: AgentState) -> Dict[str, Any]:
    """Run the full trust pipeline in just 2 LLM calls instead of 4+."""
