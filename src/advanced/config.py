...
class ResearchConfig:
    """All tunables for the research pipeline. Frozen so nodes can't
    accidentally mutate config mid-run."""
    ...
    # Planner and writer use the expensive model because output quality
    # matters most there; everything else uses mini to keep costs down.
    ...
    # Simple queries skip multi-agent overhead entirely
    ...
    # Cuts ~30% of redundant Tavily calls by deduplicating similar queries
    enable_query_dedup: bool = True
    query_similarity_threshold: float = 0.85

    # We batch these because it cuts LLM costs in half
    batch_trust_engine: bool = True
    batch_claims_credibility: bool = True

    # Stop iterating when confidence gains flatline
    ...
    # Async (not yet wired into the main flow)
    ...
    def get_query_complexity(self, query: str) -> str:
        """Rough keyword + word-count heuristic. The planner can override
        this later with a proper LLM-based classification."""
