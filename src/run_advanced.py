#!/usr/bin/env python3
"""
CLI for the advanced research pipeline.

This pipeline includes a 7-node trust engine (credibility scoring, span
verification, cross-validation), dead-end backtracking, and complexity-
based routing.

Examples:
    python -m src.run_advanced "Who is Satya Nadella?"
    python -m src.run_advanced --simple "What is quantum computing?"
    python -m src.run_advanced --single-agent "Compare Python vs JavaScript"
"""
...
from src.core.graph import (
    build_v8_graph_with_memory,
    build_v8_simple_graph,
)
...
def run_advanced(
    ...
    """Build and invoke the advanced LangGraph pipeline.  Returns the
    final report text."""
    cfg = V8Config()
    ...
    if skip_clarification:
        ...
        app, _ = build_v8_graph_with_memory(
            interrupt_on_clarify=True,
            use_multi_agent=use_multi_agent,
        )
        config = {"configurable": {"thread_id": str(uuid.uuid4())}}

    # Build the initial state with all config knobs
    init = {
    ...
    if verbose:
        print("\n" + "=" * 60)
        print("RESEARCH STUDIO — Advanced Pipeline")
        print("=" * 60)
...
def main():
    ...
    if "--help" in args or "-h" in args:
        print("""
Research Studio — Advanced Pipeline (with Trust Engine)

Usage:
    python -m src.run_advanced [OPTIONS] [QUESTION]
...
""")
    ...
    run_advanced(question, skip_clarification=skip_clarification, use_multi_agent=use_multi_agent)
