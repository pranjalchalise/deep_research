...
# Hard block obvious trash
HARD_BLOCKLIST_DOMAINS = {"oreateai.com"}

# Soft downrank (not forbidden; just less preferred)
...
# Skip deep fetch for these domains in slow mode
...
def _trust_boost(url: str) -> int:
    """
    Strongly prefer canonical/primary sources.
    """
    d = _domain(url)

    # Canonical repo gets the biggest boost
    if "github.com/langchain-ai/langgraph" in url:
        return 2000

    # General trusted domains get a big boost
...
def planner_node(state: AgentState) -> Dict[str, Any]:
    ...
    system = SystemMessage(
        content=(
            ...
            "- 5 queries exactly\n"
            "- qid must be Q1..Q5\n"
            "- Each query MUST target primary sources.\n"
            "- Avoid medium.com, reddit.com, researchgate.net unless the user asks.\n"
            "\n"
            "Use these domains heavily:\n"
            "- docs.langchain.com\n"
            "- langchain.com\n"
            "- github.com/langchain-ai/langgraph\n"
            "- langchain-ai.github.io (if present)\n"
            "\n"
            "Example query style:\n"
            "- site:docs.langchain.com langgraph stategraph messagesstate\n"
            "- site:github.com/langchain-ai/langgraph Send map reduce\n"
            "- site:docs.langchain.com langgraph interrupts human-in-the-loop\n"
            "\n"
            "No extra text."
        )
    )
...
def rank_sources_node(state: AgentState) -> Dict[str, Any]:
    ...
    # Heuristic pre-score to reduce LLM burden and enforce trust
    scored = []
    for s in sources:
        url = s["url"]
        ...
        score = snippet_len + boost - (penalty * 400)
        scored.append((score, s))

    scored.sort(key=lambda x: x[0], reverse=True)
    top = [x[1] for x in scored[: min(len(scored), 30)]]

    # LLM ranking pass for relevance ordering (not trust)
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0.0)
    system = SystemMessage(
        content=(
            "Rank sources for relevance to the user question.\n"
            "Assume trust is already handled.\n"
    ...
    ranked: List[RawSource] = []
    used = set()
    for idx in order:
        if isinstance(idx, int) and 0 <= idx < len(top) and idx not in used:
            ranked.append(top[idx])
            used.add(idx)
...
def writer_node(state: AgentState) -> Dict[str, Any]:
    ...
    system = SystemMessage(
        content=(
            ...
            "- If sources are insufficient for a detail, say so explicitly.\n"
            "- Include a short 'Gaps / Uncertain' section when needed.\n"
